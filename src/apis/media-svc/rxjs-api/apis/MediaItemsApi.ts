...
export class MediaItemsApi extends BaseAPI {
     ...
     */
    mediaItemControllerReport(
        { mediaId, reason, comment }: { mediaId: string; reason?: string; comment?: string }
    ): Observable<object>
    mediaItemControllerReport(
        { mediaId, reason, comment }: { mediaId: string; reason?: string; comment?: string },
        opts?: OperationOpts
    ...
    mediaItemControllerReport(
        ...
        throwIfNullOrUndefined(mediaId, 'mediaId', 'mediaItemControllerReport');
        const headers: HttpHeaders = {
            'Content-Type': 'application/json',
            ...(this.configuration.username != null && this.configuration.password != null ? { Authorization: `Basic ${btoa(this.configuration.username + ':' + this.configuration.password)}` } : undefined),
        };
        return this.request<object>({
            url: '/api/media-items/{mediaId}/report'.replace('{mediaId}', encodeURI(mediaId)),
            method: 'POST',
            headers,
            body: { reason, comment },
        }, opts?.responseOpts);
