...
import { imageRoot, awsUrl } from 'mediashare/core/aws/key-factory';
...
export function RNImagePickerUploader({
  uploadMode = 'photo',
  ...
  async function uploadPhoto() {
    launchImageLibrary({ mediaType: 'photo', quality: 0.5, maxWidth: 400, maxHeight: 400 }, function (res) {
      ...
      fetchAndPutToS3({ key: imageKey, fileUri: image.uri, options: { contentType: image.type } })
        .then((res) => {
