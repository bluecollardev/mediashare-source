...
import { View, Animated, TextInput as NativeTextInput, StyleSheet, I18nManager, Platform, TextStyle } from 'react-native';
...
} from 'react-native-paper/src/components/TextInput/helpers';
import { getAdornmentConfig, getAdornmentStyleAdjustmentForNativeInput } from 'react-native-paper/src/components/TextInput/Adornment/TextInputAdornment';
import { AdornmentSide, AdornmentType, InputMode } from 'react-native-paper/src/components/TextInput/Adornment/enums';
...
const TextInputFlat = ({
  ...
  const { fontSize: fontSizeStyle, fontWeight, height, paddingHorizontal, textAlign, ...viewStyle } = (StyleSheet.flatten(style) || {}) as TextStyle;
  ...
  const baseLabelTranslateX =
    (I18nManager.isRTL ? 1 : -1) * (labelHalfWidth - (labelScale * labelWidth) / 2) + (1 - labelScale) * (I18nManager.isRTL ? -1 : 1) * paddingLeft;
  ...
  // HACK! FORCE LABELS TO ALIGN TO THE TOP!
  // const inputHeight = calculateInputHeight(labelHeight, height, minInputHeight);
  const inputHeight = calculateInputHeight(labelHeight, minInputHeight, minInputHeight);
...
const Underline = ({ parentState, error, colors, activeColor, underlineColorCustom }: UnderlineProps) => {
